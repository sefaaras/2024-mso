function [bestSolution, bestFitness, iteration]=nmpa_case26(fhd, dimension, maxIteration, fNumber)
...
%  CF=4*(1-Iter/Max_iter)^(2*Iter/Max_iter);
 ...
 RB=randn(SearchAgents_no,dim);          %Brownian random number vector
  ...
  for i=1:size(Prey,1)
     for j=1:size(Prey,2)
         fdb_pos = Prey(fitnessDistanceBalance(Prey, fitness), :);
       ...
       if Iter<Max_iter/3 
% EQ 12                    A        B           C       D
          stepsize(i,j)=RB(i,j)*(fdb_pos(j)-RB(i,j)*(fdb_pos(j)));                    
%           Prey(i,j)=Prey(i,j)+P*R*stepsize(i,j); 
%                       E               F
          Prey(i,j)=fdb_pos(j)+P*R*stepsize(i,j);   
          %--------------- Phase 2 (Eqs. 13 & 14)----------------
       elseif Iter>Max_iter/3 && Iter<2*Max_iter/3 
          
         if i>size(Prey,1)/2
% EQ 13                      A         B           C       D
            stepsize(i,j)=RB(i,j)*(RB(i,j)*fdb_pos(j)-(fdb_pos(j)));
%             Prey(i,j)=Elite(i,j)+P*CF*stepsize(i,j);
%                           E               F
            Prey(i,j)=w1*fdb_pos(j)+P*CF*stepsize(i,j);
         else
% EQ 14                     A        B           C       D
            stepsize(i,j)=RL(i,j)*(fdb_pos(j)-RL(i,j)*(fdb_pos(j)));                     
%             Prey(i,j)=Prey(i,j)+P*R*stepsize(i,j);
%                           E               F
            Prey(i,j)=w1*fdb_pos(j)+P*R*stepsize(i,j);
         end  
         
         %----------------- Phase 3 (Eq. 15)-------------------
       else 
%                           A        B           C       D
           stepsize(i,j)=RL(i,j)*(RL(i,j)*fdb_pos(j)-(Prey(i,j))); 
%            Prey(i,j)=Elite(i,j)+P*CF*stepsize(i,j);
%                          E            F
           Prey(i,j)=Elite(i,j)+P*CF*stepsize(i,j);
