function [bestSolution, bestFitness, iteration]=DO_case7(fhd, dimension, maxIteration, fNumber)
...
for t=1:Maxiteration
    ...
    for i=1:Popsize
        for j=1:Dim
            dandelions_2(i,j)=dandelions(i,j)-beta(i,j)*alpha*(dandelions_mean(1,j)-beta(i,j)*alpha*dandelions(fdb_index,j)); % eq.(13) in this paper
        end
    end
