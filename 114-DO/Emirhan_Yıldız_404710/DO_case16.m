function [bestSolution, bestFitness, iteration]=DO_case16(fhd, dimension, maxIteration, fNumber)
...
for t=1:Maxiteration
    ...
    fdb_index2 = fitnessDistanceBalance( dandelions, dandelionsFitness );
    if randn()<1.5
        for i=1:Popsize
            ...
            dandelions_1(i,:)=dandelions(fdb_index2,:)+alpha.*vx.*vy.*lognpdf(lamb,0,1).*(NEW(1,:)-dandelions(i,:)); % eq.(5) in this paper
        end
    else
        for i=1:Popsize
    ...
    fdb_index = fitnessDistanceBalance( dandelions, dandelionsFitness );
    for i=1:Popsize
        for j=1:Dim
            dandelions_2(i,j)=dandelions(i,j)-beta(i,j)*alpha*(dandelions(fdb_index,j)-beta(i,j)*alpha*dandelions(i,j)); % eq.(13) in this paper
        end
    end
    ...
    fdb_index3 = fitnessDistanceBalance(dandelions,dandelionsFitness);
    for i=1:Popsize
        for j=1:Dim
            dandelions_3(i,j)=Elite(i,j)+Step_length(i,j)*alpha*(Elite(i,j)-dandelions(fdb_index3,j)*(2*t/Maxiteration)); % eq.(15) in this paper
        end
    end
