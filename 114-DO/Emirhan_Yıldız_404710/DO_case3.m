function [bestSolution, bestFitness, iteration]=DO_case3(fhd, dimension, maxIteration, fNumber)
...
for t=1:Maxiteration
    ...
    if randn()<1.5
        for i=1:Popsize
            ...
            dandelions_1(i,:)=dandelions(i,:)+alpha.*vx.*vy.*lognpdf(lamb,0,1).*(NEW(1,:)-dandelions(fdb_index,:)); % eq.(5) in this paper
        end
    else
        for i=1:Popsize
