function [bestSolution, bestFitness, iteration]=DO_case9(fhd, dimension, maxIteration, fNumber)
...
for t=1:Maxiteration
    ...
    for i=1:Popsize
        for j=1:Dim
            dandelions_3(i,j)=Elite(i,j)+Step_length(i,j)*alpha*(dandelions(fdb_index,j)-dandelions(i,j)*(2*t/Maxiteration)); % eq.(15) in this paper
        end
    end
