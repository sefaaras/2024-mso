function [bestSolution, bestFitness, iteration]=DO_case15(fhd, dimension, maxIteration, fNumber)
...
for t=1:Maxiteration
    ...
    for i=1:Popsize
        for j=1:Dim
            dandelions_2(i,j)=dandelions(fdb_index2,j)-beta(i,j)*alpha*(dandelions_mean(1,j)-beta(i,j)*alpha*dandelions(i,j)); % eq.(13) in this paper
        end
    end
    ...
    for i=1:Popsize
        for j=1:Dim
            dandelions_3(i,j)=Elite(i,j)+Step_length(i,j)*alpha*(Elite(i,j)-dandelions(fdb_index,j)*(2*t/Maxiteration)); % eq.(15) in this paper
        end
    end
