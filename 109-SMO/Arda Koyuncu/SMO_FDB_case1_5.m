...
function [bestSolution, bestFitness, iteration]=SMO_FDB_case1_5(fhd, dimension, maxIteration, fNumber)
...
for  t=1:MaxIt
    ...
    Sep_rate = log(t+ProblemSize)/(log(MaxIt)*3);    % Eq.(1)
...
%     ValSep = cec17_func(PopSep', Funcname);
    ...
    for i = 1:size(FW,2)
        WhirIdx      = Result{FW(i)};
        FDBIdx = fitnessDistanceBalance(Pop(1:FlockNum, :), Val(1:FlockNum))';
        WhirIdx = FDBIdx;
