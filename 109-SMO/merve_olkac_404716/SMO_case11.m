function [bestSolution, bestFitness, iteration]=SMO_case11(fhd, dimension, maxIteration, fNumber)
...
for  t=1:MaxIt
   %% A) Seprating strategy
    ...
    %2)  Whirling Movement
    for i = 1:size(FW,2)
        FDBIdx = fitnessDistanceBalanceIndexes(Pop(1:FlockNum, :), Val(1:FlockNum))';
        WhirIdx = FDBIdx;
        WhirlingPop = Pop(Ind(WhirIdx),:);  % Whirling Population or Xi %Whirling Equation
...
%         WhirlingVal = cec17_func(WhirlingPop',Funcname);
    ...
    for q = 1: size(FD,2)
        ...
        DivingPop = Pop(Ind(DivIdx),:);   % Diving popualtion %Diving Equation

        indexes = fitnessDistanceBalanceIndexes(DivingPop, Val(Ind(DivIdx)));
        DivingPop = DivingPop(indexes, :);
        ...
        mu  = 1;            lambda = 20;
        IGD_1 = random('InverseGaussian',mu,lambda,size(DivingPop,1),1); %Inverse Equation
        pos = find(IGD_1>1);
