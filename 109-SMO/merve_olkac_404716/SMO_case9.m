function [bestSolution, bestFitness, iteration]=SMO_case9(fhd, dimension, maxIteration, fNumber)
...
for  t=1:MaxIt
    ...
    % Use fitnessDistanceBalanceIndexes function to select indexes
    SepIndexes = fitnessDistanceBalanceIndexes(PopSep, Val(SepInd));
    
    % Selected individuals from population and best starlings set
    UnionSet = [Pop ; PosConvergance]; % Union set of the current population and the best starlings set
    Xr_hat = UnionSet(SepIndexes,:); % selected individuals
    
    % Calculate the separation operator Fsep
    ...
    % Apply separation operator to selected individuals
    Param2Change = randperm(ProblemSize, randi(ProblemSize));
    PopSep(:, Param2Change) = BestPos(Param2Change) + Fsep(:, Param2Change) .* (Xr_hat(:, Param2Change) - PopSep(:, Param2Change));
    
    % Bound correction
    PopSep = BoundCorrection(PopSep, Pop(SepInd,:), lu);
    
    % Evaluate the separated population
    ValSep = testFunction(PopSep', fhd, fNumber);
    
    % Update the new position if it is better
...
%         WhirlingVal = cec17_func(WhirlingPop',Funcname);
    ...
    for q = 1: size(FD,2)
        ...
        DivingPop = Pop(Ind(DivIdx),:);   % Diving population 
        R_D = repmat (Rep_Pos(FD(q),:),size(DivingPop,1),1); % representative member of the flock
        
        % Use fitnessDistanceBalanceIndexes function to select indexes
        DivIndexes = fitnessDistanceBalanceIndexes(DivingPop, Val(Ind(DivIdx)));
        
        % Selected individuals from population and best starlings set
        UnionSet = [Pop ; PosConvergance]; % Union set of the current population and the best starlings set
        X_j = UnionSet(DivIndexes,:); % selected individuals
        
        % Random positions selected from the current population and 
        ...
        Downward = find(DivIndexes); % Downward indexes from fitnessDistanceBalanceIndexes
        Upward = setdiff(1:size(DivingPop,1), Downward); % Upward indexes
        
        Down = zeros(size(DivingPop,1), 1);
        Down(Downward) = rand(size(Downward)); % Random values for Downward individuals
        
        UP = zeros(size(DivingPop,1), 1);
        UP(Upward) = rand(size(Upward)); % Random values for Upward individuals
        ...
        % Update the new position if it's better
