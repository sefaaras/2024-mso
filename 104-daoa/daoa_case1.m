function [bestSolution, bestFitness, iteration]=daoa_case1(fhd, dimension, maxIteration, fNumber)
...
while C_Iter<M_Iter+1                        % Main loop
   ...
   %DAF=((C_Iter/M_Iter+1)^((-1)*alpha));    %DAF1
    ...
    for i=1:size(X,1)                        % if each of the UB and LB has a just value
        for j=1:size(X,2)
            r1=rand();
            Best_FDB = fitnessDistanceBalance(X, Ffun);
            if (size(LB,2)==1)
                if r1<DAF
                    r2=rand();
                    if r2>0.5
                        ...
                        Xnew(i,j)=(Best_FDB(1,j)*DCS*((UB-LB)*Mu+LB));
                    end
                else
                    r3=rand();
                    if r3>0.5
