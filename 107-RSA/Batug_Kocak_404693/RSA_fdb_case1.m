% fitnessDistanceBalance in line 37
function [bestSolution, bestFitness, iteration]=RSA_fdb_case1(fhd, dimension, maxIteration, fNumber)
config;
LB=lbArray;
UB=ubArray;
Dim=dimension;
...
for t=1:Maxit  %Main loop %Update the Position of solutions
    ...
    new_particule = Randi + global_best + local_best
    % Instead of 'randi', use fitnessDistanceBalance
    selected_index = fitnessDistanceBalance(X, Ffun); % Get the index of the chosen guide
