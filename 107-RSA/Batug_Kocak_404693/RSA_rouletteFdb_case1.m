% rouletteFitnessDistanceBalance
function [bestSolution, bestFitness, iteration]=RSA_rouletteFdb_case1(fhd, dimension, maxIteration, fNumber)
config;
LB=lbArray;
UB=ubArray;
Dim=dimension;
...
for t=1:Maxit  %Main loop %Update the Position of solutions
    ...
    % Instead of 'randi', use rouletteFitnessDistanceBalance
    selected_index = rouletteFitnessDistanceBalance(X, Ffun); % Get the index of the chosen guide
    
    for i=2:size(X,1) 
        for j=1:size(X,2)  
                % Use the selected guide (Best_P from rouletteFitnessDistanceBalance)
