...
function[bestSolution, bestFitness, iteration] = case4(cec20so, dimension, maxFes, i)
...
for t=1:Maxit  %Main loop %Update the Position of solutions
    ...
    for i=2:size(X,1) 
        for j=1:size(X,2) 
                fdb = fitnessDistanceBalance(X,Best_F);
                R=Best_P(1,j)-X(randi([1 size(X,1)]),j)/((Best_P(1,j))+eps);
                P=Alpha+(fdb-X(i,j))/(Best_P(1,j)*(UB(j)-LB(j))+eps);
                Eta=Best_P(1,j)*P;
                if (t<T/4)
                ...
                elseif (t<3*T/4 && t>=2*T/4)
                    Xnew(i,j)=Best_P(1,j)*fdb*P*rand;
                else
                    Xnew(i,j)=Best_P(1,j)-Eta*eps-R*X(randi([1 size(X,1)]),j);
                end
        end
