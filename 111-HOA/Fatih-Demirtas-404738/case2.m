function [bestSolution, bestFitness, iteration]=case2(fhd, dimension, maxIteration, fNumber)
...
while it<MaxIt
    ...
    for i=1:nHourse
        ...
        fdb = fitnessDistanceBalance(Hourse,Hourse(i).Cost);
        
        % Update Velocity
        if CC<=0.1*nHourse

           Hourse(i).Velocity = +h_Alpha*rand(VarSize).*(GlobalBest.Position-fdb)...
