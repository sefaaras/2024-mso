function [bestSolution, bestFitness, iteration]=CO_FDB_Case4_1(fhd, dimension, maxIteration, fNumber)
            ...
            for k = 1 : m % Algorithm 1, L#10
                ...
                for j = xd % select arbitrary set of arrangements
                    ...
                    if k == 1              % The leader's step length (it is assumed that k==1 is associated to the leade number)
                        ...
                        alpha = 0.0001*t/T*abs(Xb(fitnessDistanceBalance( pop , BestSol.Cost))-X(j))+0.001.*round(double(rand>0.9));%member step length, Equation (1)%This can be updated by desired equation
                    end
