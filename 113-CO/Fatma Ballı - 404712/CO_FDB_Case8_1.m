function [bestSolution, bestFitness, iteration]=CO_FDB_Case8_1(fhd, dimension, maxIteration, fNumber)
...
%             pop(i).Cost = fobj(pop(i).Position);
        ...
        while FEs <= MaxIt % Algorithm 1, L#8
            ...
            for k = 1 : m % Algorithm 1, L#10
                ...
                X = pop(fitnessDistanceBalance( pop , BestSol.Cost)).Position;    % The current position of i-th cheetah  %% X: i'inci elemanın pozisyonunu tutar.

                X1 = pop(fitnessDistanceBalance( pop , BestSol.Cost)).Position;   % The neighbor position   %% X1: a komşusunun pozisyonunu tutar.
              
                Xb = BestSol.Position;  % The leader position  %% liderin pozisyonunu tutar.
                Xbest = X_best.Position;% The pery position  %% avın pozisyonunu tutar
                             
                kk=0;  %% değişim faktörü
                % Uncomment the follwing statements, it may improve the performance of CO
                %% Burada çitanın belirli koşullar (kendi ve avın pozisyonu vs) nasıldavranacağı modellenmiştir.
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                
                %% Burada iterasyon sayısı 1 veya 100'ün katı olduğu durumlarda rastgele permütasyonlar oluşturarak arama uzayını ve çeşitliliği artırarak algoritma performansını iyileştirme.
                ...
                %% Algorithm 1, L#12
                %% Burada çitaların adım uzunlığu hesaplanır ve rastgele parametreler belirlenir. 
                for j = xd % select arbitrary set of arrangements
                    ...
                    if k == 1              % The leader's step length (it is assumed that k==1 is associated to the leade number)
                        ...
                        alpha = 0.0001*t/T*abs(Xb(fitnessDistanceBalance( pop , BestSol.Cost))-X(fitnessDistanceBalance( pop , BestSol.Cost)))+0.001.*round(double(rand>0.9));%member step length, Equation (1)%This can be updated by desired equation
                    end
                    ...
                    if r2 <= r3              % Algorithm 1, L#15
                        ...
                        if H > r4            % Algorithm 1, L#17
                            Z(j) = X(fitnessDistanceBalance( pop , BestSol.Cost))+r_Hat.^-1.*alpha;    % Search, Equation(1) (Algorithm 1, L#18)
                        else
                            Z(j) = Xbest(fitnessDistanceBalance( pop , BestSol.Cost))+r_Check.*beta;    % Attack, Equation(3) (Algorithm 1, L#20)
                        end
                    else
                        Z(j) = X(fitnessDistanceBalance( pop , BestSol.Cost));         % Sit&wait, Equation(2) (Algorithm 1, L#23)
                    end
                end
...
%                 NewSol.Cost = fobj(NewSol.Position);
                ...
                if NewSol.Cost < pop(i).Cost
                    pop(i) = NewSol;
                    if pop(i).Cost < BestSol.Cost
                        BestSol = pop(fitnessDistanceBalance( pop , BestSol.Cost));
                    end
                end
                FEs = FEs+1;
            end
