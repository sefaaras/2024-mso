function [bestSolution, bestFitness, iteration]=CO_FDB_Case4_2(fhd, dimension, maxIteration, fNumber)
        ...
        while FEs <= MaxIt % Algorithm 1, L#8
            ...
            for k = 1 : m % Algorithm 1, L#10
                ...
                for j = xd % select arbitrary set of arrangements
                    ...
                    r1 = rand;
                    if k == 1              % The leader's step length 
                        alpha = 0.0001*t/T.*(ub(j)-lb(j)); % Step length, Equation (1)  
                    else                   % The members' step length
                        alpha = 0.0001*t/T*abs(Xb(j)-X(fitnessDistanceBalance( pop , BestSol.Cost)))+0.001.*round(double(rand>0.9));%member step length
                    end
